...
export async function POST(req: Request) {
    try {
        ...
        if (type === "login" && !existing) {
            return NextResponse.json({ success: false, error: "Sistemde böyle bir kullanıcı bulunamadı." }, { status: 404 });
        }
                  ...
                  <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
                    ...
                    <div style="background: #f8fafc; padding: 25px; border-radius: 12px; border: 1px solid #e2e8f0; text-align: center;">
                        ...
                        <p style="color: #64748b; font-size: 14px; margin-bottom: 25px;">
                            ${type === "register" ? "Kayıt işlemini tamamlamak için aşağıdaki doğrulama kodunu kullanın:" : "Sisteme güvenli giriş yapmak için aşağıdaki kodu kullanın:"}
                        </p>
